...
import { 
  ...
  Upload, FileSpreadsheet, RefreshCcw, Download, Coins, TrendingUp, AlertTriangle 
} from 'lucide-react';
...
const firebaseConfig = {
  apiKey: getEnv('VITE_FIREBASE_API_KEY') || "AIzaSyAv4ZXRI7Q7es3V3pBi_ZZce0fHvgFEKX4",
  authDomain: getEnv('VITE_FIREBASE_AUTH_DOMAIN') || "e-genlapv11.firebaseapp.com",
  databaseURL: getEnv('VITE_FIREBASE_DATABASE_URL') || "https://e-genlapv11-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: getEnv('VITE_FIREBASE_PROJECT_ID') || "e-genlapv11",
  storageBucket: getEnv('VITE_FIREBASE_STORAGE_BUCKET') || "e-genlapv11.firebasestorage.app",
  messagingSenderId: getEnv('VITE_FIREBASE_MESSAGING_SENDER_ID') || "209352282637",
  appId: getEnv('VITE_FIREBASE_APP_ID') || "1:209352282637:web:11ddcd9ae57357e7c87b44"
};
...
function AdminView({ showToast, showLoading, adminConfig, setAdminConfig, masterRhk, setMasterRhk, adminUsersData, pendingTx, setPendingTx }) {
  ...
  const handleResetDatabase = async () => {
    if (window.confirm("BAHAYA! Anda yakin mereset SEMUA data pengguna (Token, Profil, dan Riwayat Laporan)? Data ini tidak bisa dikembalikan!")) {
      showLoading(true);
      try {
        const usersRef = ref(db, 'users');
        await set(usersRef, null);
        showToast("Database Pengguna berhasil di-reset menjadi KOSONG!", "success");
      } catch (err) {
        showToast("Gagal reset database. Cek koneksi internet.", "error");
      } finally {
        showLoading(false);
      }
    }
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      <div className="flex gap-2 mb-6 bg-slate-100 p-2 rounded-xl border border-slate-200 overflow-x-auto whitespace-nowrap custom-scrollbar">
        ...
        <button onClick={() => setAdminTab('apikey')} className={`px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${adminTab === 'apikey' ? 'bg-slate-700 text-white shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>Sistem & API</button>
      </div>
      ...
      {adminTab === 'apikey' && (
        <div className={`${THEME.glossyCard} animate-in zoom-in-95`}>
          <h3 className="text-base font-bold text-slate-800 mb-3 border-b border-slate-100 pb-4 flex items-center gap-2"><Key size={18} className="text-slate-500" /> Pengaturan Sistem & API</h3>
          ...
          <form onSubmit={handleSaveApi} className="space-y-5 mb-8">
            <textarea rows="4" className={`${THEME.glossyInput} text-xs font-mono text-blue-600 bg-white`} value={String(adminConfig.geminiApiKeys)} onChange={(e) => setAdminConfig({...adminConfig, geminiApiKeys: e.target.value})} placeholder="AIzaSyA..., AIzaSyB..., AIzaSyC..." required />
          ...
          <div className="mt-8 border-t border-red-500/30 pt-6">
             ...
             <p className="text-[10px] text-slate-500 mb-4 font-medium">Mereset database akan menghapus SELURUH data pengguna (Token, Profil) dan riwayat laporan dari Firebase secara permanen.</p>
             <button onClick={handleResetDatabase} className="bg-red-50 border border-red-200 text-red-600 hover:bg-red-600 hover:text-white px-4 py-2.5 rounded-lg text-xs font-bold transition-colors shadow-sm">Reset Database Pengguna</button>
          </div>
        </div>
      )}
    </div>
