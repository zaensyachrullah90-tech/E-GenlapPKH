              ...
              <div className="animate-in fade-in slide-in-from-top-2">
                ...
                <select 
                   ...
                   onChange={e => {
                      ...
                      setRenHarId(val);
                      // LOGIKA OTOMATIS MENGISI JUDUL DAN MENGHAPUS ANGKA/TITIK DI DEPAN
                      const selectedPlan = availableRenHars.find(h => h.id === val);
                      if (selectedPlan) {
                         const cleanName = selectedPlan.name.replace(/^[\d.\s-]+/, '');
                         setDynamicData(prev => ({...prev, judul: cleanName}));
                      } else {
                         setDynamicData(prev => ({...prev, judul: ''}));
                      }
                   }} 
                   required
                >
